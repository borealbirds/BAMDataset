nll_jqpadmix = function(pars, data_in = TMB_IN_DATA, debug = FALSE) {
  ...
  int_weights_rep_dc = rep(int_weights, each = n_obs_dc)
  dr_num_dc = (r_up_rep_dc - r_lo_rep_dc) / (n_int - 1)
  
  r_int_num_dc = rep(seq(0, n_int - 1), each = n_obs_dc) / (n_int - 1) * (r_up_rep_dc - r_lo_rep_dc) + r_lo_rep_dc
  ...
  ifun_num_dc = int_weights_rep_dc * dr_num_dc * r_int_num_dc * dexp_num_fun_dc
  ...
  int_weights_rep_dd = rep(int_weights, each = n_obs_dd)
  dr_num_dd = (r_up_rep_dd - r_lo_rep_dd) / (n_int - 1)
  
  r_int_num_dd = rep(seq(0, n_int - 1), each = n_obs_dd) / (n_int - 1) * (r_up_rep_dd - r_lo_rep_dd) + r_lo_rep_dd
  ...
  ifun_num_dd = int_weights_rep_dd * dr_num_dd * r_int_num_dd * pexp_num_fun_dd
  ...
  int_weights_rep_den = rep(int_weights, each = n_obs_den)
  dr_den = r_mxs_rep / (n_int - 1)
  
  r_int_den = rep(seq(0, n_int - 1), each = n_obs_den) / (n_int - 1) * r_mxs_rep
  ...
  ifun_den = int_weights_rep_den * dr_den * r_int_den * pexp_den_fun
...
fit_jqpadmix = function(data_in,
  ...
  rtmb_data_in = data_in %>%
    as.list %>%
    c(list(int_weights = get_weights_simpson(n_integral),
