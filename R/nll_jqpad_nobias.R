jqpadnb = function(pars, data_in = TMB_IN_DATA, debug = FALSE) {
  ...
  # alpha_covs: matrix of covariate values for alpha
  ...
  alpha_q_rep = rep(alpha_q, n_int)
  
  int_weights_rep = rep(int_weights, each = n_obs)
  dr = r_max_rep / (n_int - 1)
  r_int = rep(seq(0, n_int - 1), each = n_obs) / (n_int - 1) * r_max_rep
  
  pnorm_up_fun = pnorm((log(r_up_rep) - log(r_int)) / sigma_q)
  pnorm_lo_fun = pnorm((log(r_lo_rep) - log(r_int)) / sigma_q)
  pnorm_lo_fun[r_lo_rep == 0] = 0
  pnorm_num_fun = pnorm_up_fun - pnorm_lo_fun
  pnorm_den_fun = pnorm((log(r_mxs_rep) - log(r_int)) / sigma_q)
  
  lambda_r = exp(log_lambda_q - alpha_q_rep * r_int)
