nll_jqpadrc = function(pars, data_in = TMB_IN_DATA, debug = FALSE) {
  ...
  # binom_counts: total birds counted by survey
  # binom_cmax: minimum number of total birds present during survey based on other surveys at same location and year
  # binom_to_birds_link: integer vector with length equal to length(binom_counts) that provides an index in t_up, t_lo, etc for a given survey
  # binom_to_best_link: integer vector with length equal to length(binom_counts) that provides an index in t_up, t_lo, etc for the "best" (most birds) survey conducted at that place and year
  ...
  cc_inds = c_time_inds & c_space_inds & all_inds
  dc_inds = c_time_inds & !c_space_inds & all_inds
  cd_inds = !c_time_inds & c_space_inds & all_inds
  dd_inds = !c_time_inds & !c_space_inds & all_inds
  ...
  n_obs_den = sum(all_inds)
  count_den = count[all_inds]
  
  t_max_rep = rep(t_max[all_inds], n_int)
  r_mxs_rep = rep(r_mxs[all_inds], n_int)
  
  int_weights_rep_den = rep(int_weights, each = n_obs_den) * r_mxs_rep
  r_int_den = rep(int_nodes, each = n_obs_den) * r_mxs_rep
  
  log_lambda_q_rep_den = rep(log_lambda_q[all_inds], n_int)
  alpha_q_rep_den = rep(alpha_q[all_inds], n_int)
  ...
  ## Binomial likelihood (right censoring for birds that weren't there) ----
  
  n_obs_rc = length(binom_counts)
  
  t_max_rep_rc = rep(t_max[binom_to_birds_link], n_int)
  t_mxm_rep_rc = rep(t_max[binom_to_best_link], n_int)
  r_max_rep_rc = rep(r_mxs[binom_to_birds_link], n_int)
  
  int_weights_rep_rc = rep(int_weights, each = n_obs_rc) * r_max_rep_rc
  r_int_rc = rep(int_nodes, each = n_obs_rc) * r_max_rep_rc
  
  log_lambda_q_rep_rc_this = rep(log_lambda_q[binom_to_birds_link], n_int)
  alpha_q_rep_rc_this = rep(alpha_q[binom_to_birds_link], n_int)
  lambda_r_rc_this = exp(log_lambda_q_rep_rc_this - (alpha_q_rep_rc_this * r_int_rc) ^ 2)
  
  log_lambda_q_rep_rc_best = rep(log_lambda_q[binom_to_best_link], n_int)
  alpha_q_rep_rc_best = rep(alpha_q[binom_to_best_link], n_int)
  lambda_r_rc_best = exp(log_lambda_q_rep_rc_best - (alpha_q_rep_rc_best * r_int_rc) ^ 2)
  
  pexp_rc_fun_den = 1 - exp(-lambda_r_rc_best * t_mxm_rep_rc)
  ifun_rc_den = int_weights_rep_rc * r_int_rc * pexp_rc_fun_den
  ifun_rc_mat_den = matrix(ifun_rc_den, n_obs_rc, n_int)
  
  pexp_rc_fun_num = (1 - exp(-lambda_r_rc_this * t_max_rep_rc)) * pexp_rc_fun_den
  ifun_rc_num = int_weights_rep_rc * r_int_rc * pexp_rc_fun_num
  ifun_rc_mat_num = matrix(ifun_rc_num, n_obs_rc, n_int)
  
  binom_probs = apply(ifun_rc_mat_num, 1, sum) / apply(ifun_rc_mat_den, 1, sum)
  
  ll_binom = dbinom(binom_counts, binom_cmax, binom_probs, log = TRUE)
  
  ## Bring it all together! ----
  nll_final = -(sum(ll_num_cc * count_cc) + sum(ll_num_cd * count_cd) + sum(ll_num_dc * count_dc) + sum(ll_num_dd * count_dd) - sum(ll_denom * count_den) + sum(ll_binom))
...
#
# data_in: data.frame, assumed to already have the names t_lo, t_up, t_max, t_mxm, r_lo, r_up, r_mxs, count, max_count, survey_id, best_survey_id
...
fit_jqpadrc = function(data_in,
  ...
  if (!all(data_in$best_survey_id %in% data_in$survey_id)) stop("Some survey IDs are not matched to best_survey_id in data_in. Please check your data and try again.")
  ...
  data_in_by_count = data_in %>%
    group_by(survey_id, best_survey_id) %>%
  ...
  rtmb_data_in = data_in %>%
    ...
    as.list %>%
    c(list(binom_counts = data_in_by_count$count,
           binom_cmax = data_in_by_count$max_count,
           binom_to_birds_link = match(data_in_by_count$survey_id, data_in$survey_id),
           binom_to_best_link = match(data_in_by_count$best_survey_id, data_in$survey_id),
           int_weights = integral_info$weights,
           int_nodes = integral_info$nodes,
           lambda_covs = this_lambda_covs,
           alpha_covs = this_alpha_covs))
  this_nll = function(pars) nll_jqpadrc(pars, data_in = rtmb_data_in)
  
  # for debugging
  # test = this_nll(this_init_par)
